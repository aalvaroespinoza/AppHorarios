# Arquitectura de Automatizaciones (API Routes & Server Actions)

Este documento detalla el catálogo de automatizaciones y flujos de trabajo planificados para operar como el **Sistema Nervioso Central** de LifeOS a través del backend (Next.js API Routes). Los flujos están agrupados por dominio para garantizar alta cohesión y bajo acoplamiento.
...
- **Servicios Externos:** Rutas de API usando librerías de Scraping (Cheerio/Puppeteer).
