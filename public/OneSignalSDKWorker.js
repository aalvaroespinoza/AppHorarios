...
const CACHE_NAME = `${CACHE_PREFIX}cobalt-v1`;
...
const OFFLINE_HTML = `<!doctype html><html lang="es"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><meta name="theme-color" content="#101522"><title>Sin conexión — LifeOS</title><style>*{box-sizing:border-box}body{margin:0;min-height:100dvh;display:grid;place-items:center;padding:24px;background:#f7f8fc;color:#18223a;font:16px/1.5 system-ui,sans-serif}.card{max-width:360px;padding:32px;border:1px solid #dce2ee;border-radius:20px;background:#fff}h1{font-size:26px;letter-spacing:-.04em}p{color:#5e6a82}a,button{display:inline-block;min-height:44px;padding:12px 20px;border:0;border-radius:24px;background:#3659df;color:white;font:inherit;text-decoration:none}button{margin-top:12px}@media(prefers-color-scheme:dark){body{background:#101522;color:#f1f4ff}.card{background:#181f30;border-color:#34415a}p{color:#c0c9dc}}</style></head><body><main class="card"><h1>Sin conexión</h1><p>Esta pantalla todavía no está disponible sin internet. Tus datos guardados siguen en este dispositivo.</p><a href="/">Volver a Viajes</a><br><button onclick="location.reload()">Reintentar</button></main></body></html>`;
