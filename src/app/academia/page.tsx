...
import { Bell, Plus, CheckCircle, ExternalLink, Clock, BookOpen } from 'lucide-react';
import { useAgenda } from '@/hooks/useAgenda';
import { useTracker } from '@/hooks/useTracker';
import { useEscenario } from '@/hooks/useEscenario';

export default function AcademiaPage() {
  const { diaSeleccionado, isMounted } = useEscenario();
  ...
  return (
    <motion.div 
      ...
      className="p-4 max-w-md mx-auto flex flex-col gap-6 pb-24"
      style={{ paddingTop: 'max(1rem, env(safe-area-inset-top))' }}
    >
      {/* Header */}
      <header className="flex justify-between items-end mt-2 mb-2">
        <div>
          <h2 className="text-transparent bg-clip-text bg-gradient-to-r from-emerald-400 to-cyan-400 text-[11px] font-black tracking-[0.25em] uppercase mb-1 drop-shadow-sm">
            UNIVERSIDAD & HACKING
          </h2>
        ...
        <a 
          ...
          className="flex items-center gap-1.5 px-3 py-1.5 bg-zinc-800/80 rounded-full text-zinc-300 hover:bg-zinc-700 transition-colors border border-zinc-700 shadow-sm text-sm font-medium shrink-0"
        >
      ...
      <section className="bg-zinc-900/50 border border-zinc-800/80 rounded-3xl p-5 shadow-xl backdrop-blur-md">
        <div className="flex items-center justify-between mb-4 text-zinc-400">
          <div className="flex items-center gap-2">
            <Clock size={18} />
            <h2 className="font-semibold text-sm uppercase tracking-wider">Agenda de {diaCapitalizado}</h2>
          </div>
        </div>

        <div className="relative border-l-2 border-zinc-800 ml-3 pl-5 flex flex-col gap-5 py-2">
          {agendaDelDia.length === 0 ? (
            <p className="text-zinc-500 text-sm italic">Sin eventos programados hoy.</p>
          ) : (
      ...
      <section className="bg-zinc-900/50 border border-zinc-800/80 rounded-3xl p-5 shadow-xl backdrop-blur-md">
        <div className="flex items-center gap-2 mb-4 text-zinc-400">
          <BookOpen size={18} />
          <h2 className="font-semibold text-sm uppercase tracking-wider">Progreso S4vitar</h2>
        </div>
