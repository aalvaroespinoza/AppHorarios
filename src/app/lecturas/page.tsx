...
export default function LecturasPage() {
  ...
  const getStatusColor = (estado: string) => {
    switch (estado) {
      case 'Pendiente': return 'bg-zinc-800 text-zinc-300';
      case 'En progreso': return 'bg-amber-500/20 text-amber-400 border border-amber-500/30';
      case 'Leído': return 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/30';
        ...
        <div className="flex flex-col mt-2 gap-6">
            ...
            <div key={categoria} className="flex flex-col gap-3">
              ...
              {items.map(recurso => (
                <NativeCard 
                  ...
                  className="bg-zinc-900/60 border border-zinc-800 p-4 transition-colors hover:border-zinc-700 cursor-pointer flex flex-col gap-3"
                >
                  <h4 className="text-base font-semibold text-zinc-100 leading-tight">
                    {recurso.titulo}
                  </h4>
                  <div className="flex">
          ...
          {recursos.length === 0 && (
            <NativeCard className="bg-zinc-900/40 border border-zinc-800/60 border-dashed p-8 flex flex-col items-center justify-center text-center gap-3">
              <BookOpen size={36} className="text-zinc-600 mb-1" />
