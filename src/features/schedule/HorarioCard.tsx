...
export function HorarioCard({ titulo, recomendacion, icon: Icon = Bus, direction, bec, isToday = true, diaSeleccionado = 'lunes', compact = false, onInteraction }: HorarioCardProps) {
  ...
  return (
    <motion.section layout="size" transition={{ duration: reduced ? 0 : .24, ease: [0.22, 1, 0.36, 1] }} className={'glass-panel min-w-0 overflow-hidden ' + (!compact ? 'aurora-border' : '')} aria-label={titulo}>
      {isCollapsed ? (
        ...
        <div className="p-5">
          ...
          <div className="mb-5 flex items-start justify-between gap-3">
            <div className="flex min-w-0 items-center gap-3">
              ...
              <div className="min-w-0"><p className="section-label">{isReturn ? 'Vuelta' : 'Ida'}</p><h2 className="mt-0.5 text-base font-semibold text-ink">{titulo}</h2></div>
            </div>
          ...
          {alternatives.length > 0 && (
            <div className="mt-3">
              ...
              <AnimatedDisclosure open={alternativesOpen}><div className="mt-2 flex max-h-80 flex-col gap-2 overflow-y-auto">
