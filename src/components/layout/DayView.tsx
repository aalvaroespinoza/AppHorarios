    ...
    >
      ...
      <div className="px-5 pt-5 pb-4">
        ...
        <div className="flex items-start justify-between gap-3 relative">
          <div>
            <span className="text-xs font-sans   text-muted-ink block mb-0.5">
              Tu día
            </span>
            <h2 className="text-2xl font-bold font-sans text-ink leading-snug capitalize flex items-center gap-2">
              {dateLabel.split(',')[0]}
              {scenarioId ? (
                <span className="px-2 py-0.5 rounded-2xl text-xs font-sans font-bold   border border-success/50 bg-success/15 text-success">
                  {scenario?.label}
                </span>
              ) : (
                <span className="px-2 py-0.5 rounded-2xl text-xs font-sans font-bold   border border-line bg-surface text-subtle">
                  Sin Cursada
                </span>
              )}
            </h2>
            <p className="text-xs font-sans text-subtle mt-0.5 capitalize">
          ...
          <button 
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-2xl text-xs font-sans font-bold   bg-surface text-subtle hover:text-ink hover:bg-muted transition-colors border border-line cursor-pointer shadow-none"
          >
          ...
          {showDatePicker && (
            <div className="absolute top-full right-0 mt-2 bg-surface border border-line rounded-2xl shadow-none z-50 overflow-hidden w-52 font-sans animate-in fade-in slide-in-from-top-1">
              {[0, 1, 2, 3, 4, 5, 6].map(offset => {
                ...
                return (
                  <button
                    ...
                    className={`w-full text-left px-3.5 py-2 text-xs transition-colors capitalize border-b border-line/80 last:border-0 cursor-pointer ${
                      isSelected 
                        ? 'bg-surface text-accent font-bold border-l-2 border-accent' 
                        : 'hover:bg-surface text-subtle'
                    }`}
                  >
                    {offset === 0 ? 'Hoy' : offset === 1 ? 'Mañana' : formatDateLong(d).split(',')[0]}
                    <span className="block text-xs text-muted-ink mt-0.5 capitalize">{formatDateLong(d).split(',')[1]?.trim()}</span>
                  </button>
                );
              })}
            </div>
